...
aws.init <- function(ec2key=NULL,localpubkey=NULL,optsmore=NULL){
    ...
    opts$init <- TRUE
    message(sprintf("Charging to %s, please user in options if this is incorrect",opts$user))
...
aws.clus.create <- function(name=NULL, workers=NULL,master=NULL,hadoopops=NULL,timeout=NULL,verbose=FALSE,emrfs=FALSE
    ...
    xtags <- local({
        ...
        infuse("--tags Owner='{{uusser}}' REAPER_SPARE_ME='true' crtr='rmozaws-1' {{rest}}",uusser=isn(awsOpts$user,isn(Sys.getenv("USERNAME"),"MysteriousI")),rest=rest)
    })
    ...
    class(k) <- "awsCluster"
    g <- if(wait){
        res <- (aws.clus.wait(k,mon.sec=max(5, as.integer(wait))))
...
Message     : {{currently}}
IP          : {{dns}}
SSH         : ssh hadoop@{{dns}} (assuming did aws.init(localpub=your-pub-key) \n\t else ssh -i {{pathtopriv}}  -R 9999:localhost:22 -o StrictHostKeyChecking=no hadoop@{{dns}}
